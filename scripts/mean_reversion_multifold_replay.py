"""Replay a pre-registered mean-reversion rule across multiple OOS cut dates.

This is evidence generation only.  Every fold reuses the causal signal series
and the real Paper Lab execution kernel; no fold is allowed to select a new
parameter or to grant promotion authority.
"""
...
def fold_dates(index: list[str], fractions: tuple[float, ...] = SPLIT_FRACTIONS) -> list[str]:
    """Return strictly interior, chronological cut dates from a real date index."""
    dates = list(index)
    if len(dates) < 300:
        raise ValueError("multi-fold replay requires at least 300 observations")
    cuts = [dates[int(len(dates) * fraction)] for fraction in fractions]
...
def buy_and_hold_return(series, split_date: str) -> float | None:
    """Calculate a date-aligned buy-and-hold return for the same OOS window."""
...
async def run_fold(split_date: str, frames: dict, out_dir: Path) -> dict:
    ...
    drawdown = [float(row["metrics"]["max_drawdown"]) for row in results
    ...
    return {
        ...
        "median_full_max_drawdown": float(np.median(drawdown)) if drawdown else None,
...
async def main_async() -> int:
    ...
    out_dir = Path("data/.kernel_replay_mean_reversion_multifold")
    ...
    out = Path("data/mean_reversion_multifold_replay.json")
