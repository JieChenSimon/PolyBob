      ...
      <div className="terminal-layout">
        <aside className="terminal-watchlist" aria-label={zh ? '标的队列' : 'Instrument queue'}>
          ...
          <div className="terminal-link-list">
            {links.map((item, index) => (
              <Link key={item.href} href={item.href} className={`terminal-link-row ${index === 0 ? 'selected' : ''}`}>
                <span>
        ...
        <aside className="terminal-dock">
          ...
          <div className="terminal-dock-card">
            ...
            <strong className="terminal-unknown">UNKNOWN</strong>
            <p>{zh ? '尚无标的级样本外校准结论。' : 'No instrument-level out-of-sample calibration is available.'}</p>
          </div>
          <div className="terminal-dock-card">
            <div className="terminal-card-label">{zh ? '执行权限' : 'EXECUTION PERMISSION'}</div>
            <strong className="terminal-blocked">{zh ? '已阻断' : 'BLOCKED'}</strong>
            <p>{zh ? '研究页面不会自动获得交易权限。' : 'Research pages never inherit execution permission.'}</p>
          </div>
