 ...
 * lives on the instrument's own page instead of at the top of the Equity tab —
 * where it used to render for whichever symbol happened to be the default.
 */
export default async function EquityInstrumentPage({
  params,
}: {
  ...
  return (
    <Suspense fallback={<div className="mx-auto mt-6 h-32 w-full max-w-shell px-5 md:px-8" />}>
      <InstrumentDetail symbol={decodeURIComponent(symbol).toUpperCase()} domain="us_equity" />
    </Suspense>
  );
}
