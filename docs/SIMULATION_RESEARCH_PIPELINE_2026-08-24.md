...
随后做了预先声明的主流币集合过滤（18 个 BTC/ETH/BNB/SOL 等）和趋势阈值压力：阈值 0.25 的四折绝对 OOS 中位数为 +3.99%、-15.97%、-7.94%、-10.35%，最大回撤中位数 41.94%；提高阈值到 0.50 后为 -10.73%、-4.58%、-12.54%、-10.86%，回撤约 42.85%。两者相对买入持有仍有部分正超额，但绝对成本后收益不稳定且显著为负，主流币过滤和更高阈值均淘汰，不进入模拟盘自动运行。
...
本轮继续验证了两类受约束改进。因果的事件前波动率反向加权降低了集中度（前 5 个正 PnL 标的贡献由 46.2% 降至 36.9%），但成本后收益由 9.54% 降至 7.74%，Sharpe 由 0.503 降至 0.428，因此淘汰该加权版本。更严格的 `5d / 3 insiders / $100k` 候选收益仅 1.48%、最大回撤 0.55%、Sharpe 0.551，虽然四折 OOS 均为正，但没有足够收益改善。最后的 2 倍仓位测试收益为 19.57%、最大回撤 14.88%、Sharpe 0.513、最大杠杆 1.98 倍；这只是容量/风险缩放，不是新的 alpha，不能绕过 DSR、成本和回撤门禁。对应原始 JSON 回放证据已落盘，所有版本仍为 `replay_only_not_promoted`。

随后新增的因果市场状态过滤使用 filing 前 SPY 20 个交易日收益 `>= 0`，并按事件占用率把仓位从 0.25% 调整为 0.359%，以便与原始候选比较相近的资本利用率。真实成交内核结果为收益 11.07%、最大回撤 6.18%、利润因子 1.416、年化 Sharpe 0.549，四折 OOS 分别为 11.07%、11.49%、8.01%、5.38%；前 5 个正 PnL 标的贡献约 24.7%，优于原始候选的 46.2%。这是一项可重复的风险调整改善，但 2×/3× 成本压力下 Sharpe 降为 0.396/0.243，且保守的 108 候选族 DSR 仅 0.092，仍未通过晋级门禁。该候选可以进入 Paper Lab 观察清单，但不能直接自动交易。
...
流水线现在还持久化 `pipeline_run` 元数据：命令、Git commit、PB-0027 当前状态/检查进度、PIT `as_of`、manifest 输入哈希和各数据集 coverage。小规模真实端到端冒烟已核对这些字段，且 manifest 参数按资产域记录，不再只保存最后一个域的候选。

### SEC insider 持有期优化与风险归一化复测

在同一扩展 SEC 历史和 SPY 状态过滤下，预注册比较 10/20/30 个交易日持有期。10 日候选在原仓位下收益 +4.81%、最大回撤 8.32%、Sharpe 0.256；风险归一化到 0.14% 后收益 +1.91%、最大回撤 3.32%、Sharpe 0.251，2×/3× 成本收益降至 +0.34%/-1.21%，Sharpe 0.050/-0.140，明确淘汰。30 日候选在原仓位下收益 +28.32%、最大杠杆 2.60 倍，说明不能直接把收益视为 alpha。将 30 日候选仓位降至 0.14% 后，最大杠杆约 1.02 倍、收益 +10.50%、最大回撤 5.49%、利润因子 1.733、Sharpe 0.647，四折 OOS 为 +10.43%/+11.17%/+4.90%/+3.56%；2×/3× 成本下收益 +8.89%/+7.30%，Sharpe 0.556/0.462。相较扩展 market20 基线，风险调整指标改善且集中度下降，但加入持有期等候选族后 DSR 仅 0.070，组合层月度聚类 alpha t=1.41、95% bootstrap 区间包含零，PromotionGate 仍拒绝。

另测的因果事件前波动率 `<=0.8` 过滤把回撤降至 1.18%，但收益同步降至 2.73%、最大杠杆仅 0.18 倍，属于参与度/风险缩放，不保留为改进策略。30 日 0.14% 候选仅进入 Paper Lab 观察清单，继续保持 `replay_only_not_promoted`。

随后测试了 30 日持有下的事件质量过滤：至少 3 名内部人收益 +4.48%、Sharpe 0.533；单事件金额至少 $100k 收益 +7.01%，2×/3× 成本收益 +5.81%/+4.62%，均低于未加过滤的 30 日风险归一化基线，且没有通过更高候选族的 DSR 门禁，全部淘汰。

再测试 filing date 与 transaction date 的因果披露延迟上限：7 日和 30 日候选的 1×/2×/3×成本收益分别为 +10.14%/+8.61%/+7.09% 与 +10.10%/+8.54%/+7.00%，均略低于未过滤基线；7 日候选组合层聚类 alpha t=1.39，95% bootstrap 区间仍跨零，且 648 候选族 DSR/成本门禁失败。延迟过滤正式淘汰，当前最佳观察候选仍为未加延迟过滤的 30 日风险归一化版本。

最后对市场状态做互补分层：SPY 20 日收益 `<=0` 的弱市场事件收益 +7.41%、Sharpe 0.529，2×/3×成本收益 +6.34%/+5.28%；更弱的 `<=-5%` 事件收益 +3.22%、Sharpe 0.425，四折 OOS 后两折仅 +0.66%/+0.19%。弱市场 0 分支组合 alpha t=0.72、正 alpha 月份仅 37.7%，两类弱市场状态均淘汰，支持当前策略只在非弱市场状态观察而非在所有市场自动运行。
