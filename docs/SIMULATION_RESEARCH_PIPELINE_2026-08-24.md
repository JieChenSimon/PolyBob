...
新增 [cross_sectional_local_screen.json](../data/cross_sectional_local_screen.json)，在本地全量日线宇宙上测试预注册的 54 个 long-only 横截面动量组合（20/30/60 日回看，赢家占比 20%/30%，每日/每 5 日/每 10 日换仓，原始/10% 年化波动目标/带回撤熔断），只用当日以前的收益排序并扣除域成本。

- 美股原始 2715 个标的中有 781 个因单日价格跳变超过 1.5 倍被质量门禁剔除，剩余 1934 个标的上 18 个候选重新计算；`lookback=60, top_frac=0.3, rebalance_days=10` 单次 OOS 为 +27.30%、相对基准 +2.69%，但 8 个滚动折的相对收益中位数约为 0，不能通过稳定性门禁。
...
受约束的换手优化中，质量门禁修复前出现的 +108.04% 首折收益已经消失，证明此前结果被异常价格污染。质量门禁后该候选平均换手约 0.060，20% 单日收益截断压力情景 OOS 仍为 +26.82%，但 8 折中仍有 4 折跑输基准，滚动中位数约为 0，暂不进入模拟盘；下一步仍需做公司行动复核、幸存者偏差、block bootstrap、DSR/PBO 和真实模拟盘长时间 replay。
