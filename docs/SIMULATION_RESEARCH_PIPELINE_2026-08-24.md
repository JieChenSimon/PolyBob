...
本轮实际跑的是 `momentum_dualma_v1` 的 4 组预注册参数候选，不等同于系统中所有策略都已经验证。`signal_fusion`、价差均值回归/套利以及依赖资金费、盘口、SEC 事件的策略仍需各自接入兼容的真实数据回放和独立 OOS 门禁；报告不把它们伪装成已完成。
