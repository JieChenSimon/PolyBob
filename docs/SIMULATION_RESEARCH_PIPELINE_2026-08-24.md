...
本轮的候选选择只是诊断用途。训练期选出的参数为：A 股 `mom_10_40_12`，美股 `mom_8_32_10`，加密货币 `mom_8_32_10`。多重检验门槛记录了 4 次候选试验，对应 deflated t 门槛约 3.30。最终共同 OOS 收益矩阵的 PBO 为：A 股 0.5992（overfit）、美股 0.7738（overfit）、加密货币 0.6389（overfit），整体晋级状态仍为 BLOCKED。

本轮新增的每标的平均成本归因（初始资金 $10,000）：A 股手续费约 $37.37、滑点约 $7.36；美股手续费约 $40.98、滑点约 $104.10；加密手续费约 $55.60、滑点约 $129.43。成本确实显著，但美股中位数收益仍为 -1.29%、中位数 profit factor 约 0.60，说明不能只靠降费或调滑点解决，当前信号方向本身没有稳定的费用后优势。
