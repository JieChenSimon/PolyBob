...
signal source (see :mod:`services.simulation.sources`) to a universe of
...
DEFAULT_RUN_CONFIG: dict[str, Any] = {
    ...
    # Feedback guardrails (see services/simulation/metrics.py docstring).
    "auto_feedback": False,
...
class _ActiveRun:
    ...
    __slots__ = ("record", "source", "marks", "last_trade_at", "lock",
...
class SimulationService:
    ...
    def _equity_snapshot(self, active: _ActiveRun) -> tuple[float, float, dict[str, float]]:
        """Return (equity, gross_exposure, per-instrument abs notional)."""
    ...
    async def _process_signal(self, active: _ActiveRun, signal: SimSignal) -> None:
        async with active.lock:
            ...
            equity, _, notionals = await asyncio.to_thread(
    ...
    async def _record_equity(self, active: _ActiveRun) -> None:
        equity, gross, _ = await asyncio.to_thread(self._equity_snapshot, active)
        now = datetime.now(UTC)
