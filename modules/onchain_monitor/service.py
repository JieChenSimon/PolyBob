...
class OnchainMonitorService:
    ...
    def __init__(
        ...
        # How often an unpriced leg made a cluster total unknowable. Surfaced in
        # the summary so a silent gap in price coverage is visible rather than
        # showing up as "no alerts today".
        self._undetermined_cluster_windows = 0
    ...
    def get_summary(self) -> dict:
        ...
        # A total that silently skips the unpriced legs is not a total. If any
        # recent transfer has no USD quote, the aggregate is unknown and says so.
        undetermined = [e for e in recent_events if e.usd_value is None]
        undetermined_usd_events = sum(
            1 for e in self.events.values() if e.usd_value is None
        )
        recent_cex_flow_usd = (
            None if undetermined else sum(e.usd_value for e in recent_events)
        )
        ...
        return {
            "undetermined_usd_events": undetermined_usd_events,
            "undetermined_cluster_windows": self._undetermined_cluster_windows,
    ...
    async def ingest_event(self, payload: dict | OnchainTransferEvent) -> dict:
        ...
        return {
            ...
            # Told to the caller directly, so an unpriced transfer is visible at
            # ingest rather than only as an alert that never fired.
            "undetermined_usd_value": event.usd_value is None,
        }
    ...
    def _build_event(self, payload: dict) -> OnchainTransferEvent:
        ...
        if isinstance(block_time, str):
            ...
            # Substituting local wall-clock for a chain timestamp silently
            # shifts the event into or out of the cluster window, which is the
            # whole basis of the distribution alert. An event without a block
            # time is not an event we can reason about.
            raise ValueError("block_time is required; a chain event without one cannot be placed in time")
        ...
        return OnchainTransferEvent(
            ...
            # An unpriced transfer is unknown, not zero. Recording $0 made a
            # genuine large distribution invisible to the threshold that exists
            # to catch exactly that.
            usd_value=(float(payload["usd_value"])
                       if payload.get("usd_value") is not None else None),
    ...
    async def _evaluate_event(self, event: OnchainTransferEvent) -> list[DistributionAlert]:
        ...
        if event.usd_value is None:
            # Every rule below is a comparison against a USD threshold. An
            # unpriced transfer cannot clear or fail one, and treating it as $0
            # meant a real distribution passed through silently. It is recorded
            # and counted; it just cannot be judged.
            if watch is not None:
                # It landed on a watched address, so it belongs to a cluster
                # window whose total is now unknowable — not merely quiet.
                self._undetermined_cluster_windows += 1
            logger.info("onchain_event_unpriced", tx_hash=event.tx_hash,
    ...
    def _build_cluster_alert(self, watch: OnchainWatchAddress) -> DistributionAlert | None:
        ...
        if any(event.usd_value is None for event in cex_events):
            # One unpriced leg makes the cluster total unknown. Summing the rest
            # and comparing to a threshold would systematically under-report:
            # the missing leg can only push the total up, never down.
            self._undetermined_cluster_windows += 1
            return None
