...
| A 股 | 8 | 8 | 0 | 8 | 0 |
| 加密 | 18 | 36 | 0 | 0 | 36 |
| 深回撤跨域案例 | 43 | 516 | 0 | 102 | 414 |
| 合计（去重标的） | 79 | 587 | 0 | 137 | 450 |

当前结果不能证明任何标的达到用户目标。美股和 A 股的独立固定资本回放已经有
年/月目标失败证据；加密多折和最新 walk-forward 回放有 OOS 结果，但缺少同口径的年化/月度目标与
稳定性字段，因此保持 UNKNOWN。深回撤结果现在按标的、持有期和成本倍数逐行纳入；
没有事件或没有 OOS 完整事件的行保持 UNKNOWN，不能因权益曲线没有变化而记成 FAIL。
所有交易的历史盘口深度仍未达到可执行证据门禁。

## 下一步

1. 为加密逐标的回放接入与股票相同的固定资本、月度目标和三折稳定性合同。
2. 对 137 个有实际失败证据的案例按交易成本、换手、负贡献时期和最大回撤分解，候选只能
   使用训练区间预注册，不能用 OOS 反选。
3. 对 450 条 UNKNOWN 证据优先补齐真实历史、费率和成交质量数据；在证据补齐前
   不调参、不晋级。
