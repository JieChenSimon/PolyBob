...
针对连续负贡献标的，新增只使用当前及历史价格的固定止损/冷却层，并以训练期
选择 `stop_loss_pct=0.10, cooldown_bars=5`；随后使用相同严格 OOS 和真实
`SimulationService` 隔离回放复测：

| 域/版本 | 中位数 | 均值 | 正收益数 | 总收益低于 -30% | 稳定通过 |
|---|---:|---:|---:|---:|---:|
| 美股无保护 | +5.17% | +10.54% | 18/27 | 2 | 14/27 |
| 美股止损 10% + 冷却 5 | +5.17% | +11.72% | 19/27 | 1 | 13/27 |
| A 股无保护 | -2.31% | +0.46% | 4/8 | 0 | 3/8 |
| A 股止损 10% + 冷却 5 | -2.31% | +0.46% | 4/8 | 0 | 3/8 |

该保护层减少了美股极端亏损并提高均值，但没有改善中位数，且稳定性略降；
A 股没有实质变化。因此它只能作为风险控制候选，不能被描述为收益优化，
也不能晋级真实交易。产物：
`data/cross_sectional_standalone_us_oos_candidate_20_30_5_stop10_cd5.json`、
`data/cross_sectional_standalone_a_oos_candidate_20_30_5_stop10_cd5.json`。
